...
buildTypes schemaType schemaFilePath queryFilePaths = do
    ...
    queryTypes <-
        fmap nubOrd . sequence $
            concatMap errorOnLeft $
                sequence $
                    mapM
    ...
    mkEnumDefs ns = do
        decs <- forM ns $ \n -> case lookUpTypeInSchema sd n of
            Just (GQL.TypeDefinitionEnum GQL.EnumTypeDefinition{..}) -> do
                let typeNameStr = T.unpack . GQL.unName $ n
        ...
        Just (GQL.Name n) -> do
            ...
            case GQL._todVariableDefinitions tod of
                ...
                varDefs -> do
                    ...
                    let typeDef = mkNewtypeForRecords queryName records
...
lookUpRootOperationTypeDefinition tds opt =
  ...
  where
    findInSchemaDefinitions = listToMaybe $
        catMaybes $
            flip map tds $ \case
...
lookUpTypeInSchema _ n@(GQL.Name "Boolean") =
    pure $ GQL.TypeDefinitionScalar $ GQL.ScalarTypeDefinition Nothing n []
lookUpTypeInSchema tds n = listToMaybe $
    catMaybes $
        flip map tds $ \case
